...
- [상품 검색 API](#%EC%83%81%ED%92%88-%EA%B2%80%EC%83%89-api)
    - [상품 검색](#%EC%83%81%ED%92%88-%EA%B2%80%EC%83%89)
- [상품 상세 조회 API](#%EC%83%81%ED%92%88-%EC%83%81%EC%84%B8-%EC%A1%B0%ED%9A%8C-api)
    - [상품 단건 조회](#%EC%83%81%ED%92%88-%EB%8B%A8%EA%B1%B4-%EC%A1%B0%ED%9A%8C)
- [선착순 쿠폰 기능](#%EC%84%A0%EC%B0%A9%EC%88%9C-%EC%BF%A0%ED%8F%B0-%EA%B8%B0%EB%8A%A5)
...
- [주문 / 결제 API](#%EC%A3%BC%EB%AC%B8--%EA%B2%B0%EC%A0%9C-api)
    - [주문/결제 생성](#%EC%A3%BC%EB%AC%B8%EA%B2%B0%EC%A0%9C-%EC%83%9D%EC%84%B1)
    - [주문/결제 취소](#%EC%A3%BC%EB%AC%B8%EA%B2%B0%EC%A0%9C-%EC%B7%A8%EC%86%8C)
- [상위 상품 조회 API](#%EC%83%81%EC%9C%84-%EC%83%81%ED%92%88-%EC%A1%B0%ED%9A%8C-api)
    - [인기 상품 조회](#%EC%9D%B8%EA%B8%B0-%EC%83%81%ED%92%88-%EC%A1%B0%ED%9A%8C)
- [장바구니 기능](#%EC%9E%A5%EB%B0%94%EA%B5%AC%EB%8B%88-%EA%B8%B0%EB%8A%A5)
...
- [잔액 충전 / 조회 API](#%EC%9E%94%EC%95%A1-%EC%B6%A9%EC%A0%84--%EC%A1%B0%ED%9A%8C-api)
...
{
  "userId": 123,
  "couponType": "RATE",       // "AMOUNT" or "RATE"
  "discountValue": 10,        // 정률 할인 시 10%면 10, 금액 할인 시 10000원이면 10000
  "couponId": 101             // 발급받고자 하는 쿠폰 식별자
}
```

| 필드명           | 타입     | 설명                         |
| ------------- | ------ | -------------------------- |
| userId        | number | 사용자 식별자(필수)                |
...
- **Endpoint**: `GET /coupons?userId={userId}`
...
- **Description**: 사용자 식별자, 상품 목록(상품 ID, 수량), (선택) 쿠폰 ID 등을 입력받아 결제를 수행한다.
...
```json
{
  ...
  "discountAmount": 300000,    // 쿠폰이 적용된 할인 금액 (선택)
  "totalAmount": 3000000,     // 할인 적용 후 최종 결제 금액
  "balance": 0,               // 결제 후 잔액
  "orderDateTime": "2025-01-27T15:00:00Z",
  "status": "PAID"
}
```
...
- **Endpoint**: `POST /carts`
- **Description**: 사용자 식별자와 상품 ID, 수량을 받아 장바구니에 추가한다.
...
- **Endpoint**: `GET /carts?userId={userId}`
- **Description**: 특정 사용자의 장바구니 목록을 조회한다.
...
|파라미터명|타입|설명|
|---|---|---|
|cartItemId|number|장바구니 상품 식별자(필수)|
...
| ------- | ------ | ----------- |
| userId  | number | 사용자 식별자     |
| balance | number | 충전 후 사용자 잔액 |

### 잔액 조회

- **Endpoint**: `GET /balances/{userId}`
- **Description**: 사용자 식별자를 경로로 받아 해당 사용자의 현재 잔액을 조회한다.
